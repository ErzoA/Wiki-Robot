...
async def hmm(_, message):
    ...
    if status == "ON" or status == "on" or status == "On":
        ...
        if not lol:
            ...
            return
        await lel.edit(
            f"Muscat AI Actived by {message.from_user.mention()} for users in {message.chat.title}"
        )
        ...
        if not Escobar:
            ...
            return
        await lel.edit(
            f"Muscat AI Deactivated by {message.from_user.mention()} for users in {message.chat.title}"
        )
...
async def hmm(client, message):
    ...
    if chat_id in en_chats:
        ...
        test = test.replace(
            "I live in @WikiTapiGroup.", "I live in San Francisco, California."
        )
...
@pbot.on_message(
    filters.regex("Muscat|muscat|robot|MUSCAT|cat|Cat|Wik|wikwik|Wikwik|wik")
    & ~filters.bot
    & ~filters.via_bot
    & ~filters.forwarded
    & ~filters.reply
